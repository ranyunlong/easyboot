...
export class Layer {
    ...
    public async parseParamMetadata(context: Context) {
        ...
        if (paramMetadata) {
            ...
            if (Array.isArray(paramtypes)) {
                ...
                const validations =  Reflect.getMetadata(MetadataEnums.Base.VALIDATORS, Entity.prototype)
                if (Array.isArray(validations)) {
                    ...
                    validations.forEach(({validation, propertyKey}) => {
                        const metatype = Reflect.getMetadata(MetadataEnums.Base.TYPE, Entity.prototype, propertyKey);
    ...
    public async parseQueryMetadata(context: Context) {
        const bodyMetadata = Reflect.getMetadata(MetadataEnums.Controller.REQUEST_QUERY, this.Controller, this.propertyKey)
        if (bodyMetadata) {
