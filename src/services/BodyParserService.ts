...
import { FormMidableConfiguration } from '../configurations/FormMidableConfiguration';
import { ServletContext } from '../core/ServletContext';
import { BodyParseConfiguration } from '../configurations/BodyParseConfiguration';
...
export class BodyParserService {
    ...
    constructor(public opts: BodyParseConfiguration) {
     ...
     */
    public async parseBody(ctx: ServletContext) {
        let body: any = {}
     ...
     */
    public async parseFile(ctx: ServletContext, options?: UploadOptions): Promise<{
        [key: string]: any;
    } | undefined> {
        if (/(GET|DELETE|HEAD|COPY|PURGE|UNLOCK)/.test(ctx.method) && this.strict) {
            showWarn()
